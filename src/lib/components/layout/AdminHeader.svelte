...
>
  <div class="flex items-center justify-between h-full px-4 lg:px-6">
    <div class="flex items-center gap-4">
      ...
      <div>
        <h1 class="text-lg font-display font-semibold text-surface-900">
          {pageTitle}
        </h1>
        <p class="text-sm text-surface-500 hidden sm:block">
          Manage your college convenience store
        </p>
      </div>
    </div>
    ...
    <div class="flex items-center gap-3">
      <a
        href="/"
        class="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium text-surface-600 hover:bg-surface-100 transition-colors"
      >
        <svg
          class="w-4 h-4"
          fill="none"
          ...
          {#if menuOpen}
            ...
            >
              ...
              <a
                ...
                onclick={closeMenu}>View Store</a
              >
              <hr class="my-1 border-surface-100 sm:hidden" />
              <a
                href="/logout"
                class="block px-4 py-2 text-sm text-red-600 hover:bg-red-50"
                onclick={closeMenu}>Sign Out</a
              >
            </div>
          {/if}
        </div>
