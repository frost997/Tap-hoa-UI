...
>
  ...
  <div class="flex items-center gap-3 px-4 h-16 border-b border-secondary-800">
    <div
      class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-400 to-primary-600 flex items-center justify-center flex-shrink-0"
    >
  ...
  <nav class="flex-1 overflow-y-auto py-4">
    {#each navGroups as group}
      {#if ui.sidebarOpen}
        <p
          class="px-4 mb-2 text-xs font-semibold text-secondary-400 uppercase tracking-wider"
        >
          {group.label}
        </p>
      {/if}
...
>
  <div
    class="flex items-center justify-between px-4 h-16 border-b border-secondary-800"
  >
    <div class="flex items-center gap-3">
      <div
  ...
  <div class="border-t border-secondary-800 p-4">
    <a
      href="/"
      class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-secondary-300 hover:bg-secondary-800 hover:text-white transition-colors"
    >
      <svg
        class="w-5 h-5"
        fill="none"
